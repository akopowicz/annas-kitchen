...
// export default function HeartCard({ recipeId }) {
...
//   return (
//     <div
//       className="bg-white absolute top-0 right-0 m-2 drop-shadow p-1 rounded-[50%] z-10"
//       onClick={handleClick}
//     >
//       <svg
//         width="30"
...
export default function HeartCard({ recipeId }: { recipeId: string | number }) {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      if (localStorage.getItem(recipeIdString)) {
        setIsFilled(true);
      }
    }
  }, [recipeId]);
  ...
  const handleClick = () => {
    ...
    if (typeof window !== "undefined") {
      if (localStorage.getItem(recipeIdString)) {
        localStorage.removeItem(recipeIdString);
      } else {
        localStorage.setItem(recipeIdString, recipeIdString);
      }
    }
  };
